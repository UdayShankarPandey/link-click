...
import { LogIn, Mail, Lock, AlertCircle, RefreshCw, Eye, EyeOff } from 'lucide-react';
...
const Login = () => {
  ...
  const [resendCooldownEndTime, setResendCooldownEndTime] = useState(null);
  ...
  // Timestamp-based resend cooldown timer effect (immune to interval drift / tab throttling)
  useEffect(() => {
    ...
    const updateTimer = () => {
      const now = Date.now();
      const remaining = Math.max(0, Math.ceil((resendCooldownEndTime - now) / 1000));
      setResendCooldown(remaining);
      if (remaining <= 0) {
        setResendCooldownEndTime(null);
      }
    };
  ...
  const startResendCooldown = (seconds = 60) => {
    setResendCooldownEndTime(Date.now() + seconds * 1000);
  };
